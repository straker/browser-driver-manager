...
urlMacOsStable="https://download.mozilla.org/?product=firefox-latest-ssl&os=osx&lang=en-US"
urlMacOsBeta="https://download.mozilla.org/?product=firefox-beta-latest-ssl&os=osx&lang=en-US"
urlMacOsDev="https://download.mozilla.org/?product=firefox-devedition-latest-ssl&os=osx&lang=en-US"
urlMacOsNightly="https://download.mozilla.org/?product=firefox-nightly-latest-ssl&os=osx&lang=en-US"
...
echo "Installing Firefox $(titleCase $channel) (this may take awhile)"
...
if [ $BDM_OS == "Linux" ]; then
  ...
  if command -v dpkg >/dev/null; then
    ...
    error "Unable to download Firefox; System does not support .dep or .rpm"
    exit 1
  fi
...
echo "Successfully installed $($BDM_SRC_DIR/version/firefox.sh $channel)"